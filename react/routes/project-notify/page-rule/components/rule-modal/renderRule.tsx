...
import { Select, DatePicker, TimePicker, DateTimePicker, TextArea, TextField, NumberField, DataSet } from 'choerodon-ui/pro';
...
const renderRule = (dataset: DataSet, fieldK: IFieldK, fieldData: IField[], systemDataRefMap: React.MutableRefObject<Map<string, any>>) => {
    ...
    if(operation === 'is' || operation === 'is_not' ) {
      return (
        <Select required name={`${key}-value`} placeholder="值">
    ...
    if(field) {
      ...
      if(system) {
          switch(code) {
              ...
              // case 'backlogType': {
              //   return <SelectDemandType required name={`${key}-value`} placeholder="值" />
              // }
              // case 'backlogClassification': {
              //   return <SelectTreeDemandClassification required name={`${key}-value`} placeholder="值" />
              // }
              // case 'urgent': {
              //   return <SelectUrgent required name={`${key}-value`} placeholder="值" />
              // }
          }
      }
      switch(fieldType) {
          ...
          case 'member': {
              return (
                <SelectUser required name={`${key}-value`} placeholder="值" />
              )
          }
          case 'text': {
              <TextArea
          ...
          case 'date': {
              return (<DatePicker required name={`${key}-value`} placeholder="值" />)
          }
          default:
    ...
    return (
      <Select name={`${key}-value`} required placeholder="值" />
    )
}
