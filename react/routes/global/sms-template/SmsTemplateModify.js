...
@inject('AppState')
@observer
export default class SmsTemplateModify extends Component {
  ...
  getBackPath = () => {
    ...
    if (this.mail.type === 'organization') {
      backPath = `/sms/sms-template?type=${type}&id=${orgId}&name=${orgName}&organizationId=${orgName}`;
    } else {
      backPath = '/sms/sms-template';
    }
    return backPath;
  }
  ...
  handleSave = (values) => {
    ...
    MailTemplateStore.updateTemplateDetail(MailTemplateStore.getCurrentDetail.id, body, type, orgId)
      .then((data) => {
        if (data.failed) {
          Choerodon.prompt(data.message);
        } else {
      ...
      <Form className="c7n-mailtemplate-create-form">
        ...
        >
          {getFieldDecorator('businessType', {
            ...
            initialValue: MailTemplateStore.getCurrentDetail.businessType,
          })(
            <Select
        ...
        >
          {
            getFieldDecorator('smsContent', {
              ...
              initialValue: MailTemplateStore.getCurrentDetail.smsContent || undefined,
            })(
              // <Input autoComplete="off" style={{ width: inputWidth }} label="内容" />,
              <TextArea
                label="内容"
                placeholder="内容必须为Json格式"
                style={{ width: inputWidth }}
  ...
  render() {
    return (
      <Page>
        ...
        />
        <Content
          title={`修改短信模板${MailTemplateStore.getCurrentDetail.code}`}
          description="修改短信模板可以让您根据自己的需求，修改发送的主题、内容。"
        >
          {this.renderContent()}
        </Content>
      </Page>
